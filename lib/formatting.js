  ...
  if (options.averages) {
    if (!heapHeaders.length) {
      ...
      options.heapAverages.forEach((heapCount) => {
        const headerParts = JSON.parse(heapHeaderPart)
...
export function getStats(options, stats) {
  ...
  for (const key in memwatchHeader) {
    ...
    // copy memwatch header col to inherit settings
    const col = { ...memwatchHeader[key] }
...
export function getHeapAverages(options, stats) {
  ...
  ui.div(...cols.map((colValue, index) => {
    // copy heap header col to inherit settings
    const col = { ...heapHeaders[index] }
...
export function getDiff(diff) {
  ...
  details.sort(sortHeapDiffDetails)
